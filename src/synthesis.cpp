...
void Synthesize(const double *f0, int f0_length, double **const spectrogram,
  ...
  double **aper_bufs = new double *[f0_length];
  for (int i = 0; i < f0_length; ++i) {
    aper_bufs[i] = new double[fft_size / 2 + 1];
  }
  ...
  for (int i = 0; i < f0_length; ++i) {
    ...
    for (int k = 0; k < number_of_aperiodicities; ++k) {
        coarse_aperiodicity[k+1] = aperiodicity[i][k];
    }
    interp1(coarse_frequency_axis, coarse_aperiodicity,
      number_of_aperiodicities + 2, frequency_axis, fft_size / 2 + 1, aper_bufs[i]);
    for (int j = 0; j <= fft_size / 2; ++j)
      aper_bufs[i][j] = pow(10.0, aper_bufs[i][j] / 20.0);
  }
  
  // Synthesis
  Synthesis(f0, f0_length, spectrogram, aper_bufs, fft_size, frame_period, fs, y_length, y);
